...
// Helper function to create and connect server for standalone mode
export async function startStandaloneServer() {
    ...
    // Register all tools (duplicate registration for standalone mode)
    registerTools(server, hfClient)

    // Connect using stdio transport
    const transport = new StdioServerTransport()
    await server.connect(transport)
    console.error('MCP server started in standalone mode')
}

// Helper function to register all tools
function registerTools(server: McpServer, hfClient: InferenceClient | null) {
    // 인사말 도구
    server.registerTool(
        'greet',
        {
    ...
    server.registerTool(
        ...
        async ({ num1, num2, operator }) => {
            let resultText = ''
            switch (operator) {
                case '+': resultText = `${num1} + ${num2} = ${num1 + num2}`; break
                case '-': resultText = `${num1} - ${num2} = ${num1 - num2}`; break
                case '*': resultText = `${num1} * ${num2} = ${num1 * num2}`; break
                case '/': resultText = num2 === 0 ? '오류: 0으로 나눌 수 없습니다.' : `${num1} / ${num2} = ${num1 / num2}`; break
            }
            return createTextResponse(resultText)
        }
    )
    ...
    server.registerTool(
        'get-time',
        {
            description: '타임존을 입력받아 해당 타임존의 현재 시간을 반환합니다.',
            inputSchema: z.object({
                timezone: z.string().describe('IANA 타임존 이름')
            })
        },
        async ({ timezone }) => {
            try {
                ...
                return createTextResponse(`${timezone}의 현재 시간: ${formatter.format(new Date())}`)
            } catch {
                return createTextResponse(`오류: 유효하지 않은 타임존입니다. (${timezone})`)
            }
        }
    ...
    server.registerTool(
        'geocode',
        {
            description: '도시 이름이나 주소를 입력받아 위도와 경도 좌표를 반환합니다.',
            inputSchema: z.object({
                address: z.string().describe('도시 이름 또는 주소')
            })
        },
        async ({ address }) => {
            try {
                const url = `https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(address)}&format=jsonv2&limit=1`
                const response = await fetch(url, { headers: { 'User-Agent': 'MCP-Geocode-Tool/1.0' } })
                const data = await response.json()
                if (!Array.isArray(data) || data.length === 0) {
                    return createTextResponse(`주소를 찾을 수 없습니다: ${address}`)
                }
                const { lat, lon, display_name } = data[0]
                return createTextResponse(`주소: ${display_name}\n위도: ${lat}\n경도: ${lon}`)
            } catch (error) {
                return createTextResponse(`오류: ${error instanceof Error ? error.message : String(error)}`)
            }
        }
    )
    ...
    server.registerTool(
        'get-weather',
        {
            description: '위도와 경도 좌표로 날씨 정보를 제공합니다.',
            inputSchema: z.object({
                latitude: z.number().min(-90).max(90).describe('위도'),
                longitude: z.number().min(-180).max(180).describe('경도'),
                forecastDays: z.number().int().min(1).max(16).optional().default(7).describe('예보 기간')
            })
        },
        async ({ latitude, longitude, forecastDays = 7 }) => {
            try {
                ...
                let text = `📍 위치: (${latitude}, ${longitude})\n🌡️ 현재: ${current.temperature}°C, ${getWeatherDescription(current.weathercode)}\n\n📅 예보:\n`
                for (let i = 0; i < Math.min(forecastDays, daily.time.length); i++) {
    ...
    server.registerTool(
        'generate-image',
        {
            ...
            inputSchema: z.object({
                prompt: z.string().describe('생성할 이미지 설명')
            })
        },
        async ({ prompt }) => {
            if (!hfClient) {
                return createTextResponse('오류: HF_TOKEN이 설정되지 않았습니다.')
            }
            try {
                const image = (await hfClient.textToImage({
                    provider: 'auto',
